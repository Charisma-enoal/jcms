...
public class ExcelUtil {
    ...
    public static <T> T readDataT(Workbook wb, T t, InputStream in, Integer[]... integers)
    ...
    public static <T> List<T> readDataListT(Workbook wb, T t, int beginLine, int totalcut)
            throws Exception {
        ...
        try {
            if (file.exists()) {
                ...
                for (int m = 0; m < declaredFields.length; m++) { // 设置title
                    if(declaredFields[m].isAnnotationPresent(IsNeeded.class)){
                ...
                for (int i = 0; i < list.size(); i++) { // 数据
                    ...
                    for (int j = 0; j < declaredFields.length; j++) {
                        if(declaredFields[j].isAnnotationPresent(IsNeeded.class)){
    ...
    public static <T> void exportExcel(String fileName,
        ...
        try{
            ...
            response.reset();
            String pFileName = encodeChineseDownloadFileName(request,fileName) + ".xlsx";
